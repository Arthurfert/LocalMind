...
- Always wrap mathematical expressions in $ or $$ delimiters."""
    ...
    def chat_stream(
        self, model, messages, chunk_callback=None, images=None, tools=None
    ):
        """
        ...
        Args:
            ...
            tools: liste d'outils MCP formattés dictionnaire
        """
        try:
            import json
            ...
            if tools:
                # Injection manuelle des schémas d'outils dans le prompt système pour parer
                # aux modèles dont le template par défaut gère mal les "tools"
            ...
            payload = {"model": model, "messages": messages_with_system, "stream": True}
            if tools:
                payload["tools"] = tools
            ...
            if response.status_code == 200:
                ...
                self.current_response = None
                return {"content": f"Erreur: {response.status_code}", "tool_calls": []}

        except requests.exceptions.Timeout:
            return {
                "content": "Erreur: La requête a expiré. Le modèle met trop de temps à répondre.",
                "tool_calls": [],
            }
        except Exception as e:
            return {"content": f"Erreur: {str(e)}", "tool_calls": []}
