...
#[tauri::command]
fn change_current_dir(new_path: String) -> Result<String, String> {
    match std::env::set_current_dir(&new_path) {
        Ok(_) => Ok(new_path),
        Err(e) => Err(format!("Impossible de changer le dossier : {}", e)),
    }
}
                        ...
                        for tc in tool_calls {
                            if let Some(func) = tc.get("function") {
                                ...
                                if let Some(server_name) = tool_server_map.get(func_name) {
                                    println!(
                                        "-> Appel fonction MCP: {} sur {}",
                                        func_name, server_name
                                    );
                                    let mut res =
                                        mcp.call_tool(server_name, func_name, args.clone()).await;

                                    // Gestion de la demande de confirmation
                                    if let Some(r) = &res {
                                        if let Some(demand) =
                                            r.get("confirmation_demand").and_then(|d| d.as_str())
                                        {
                                            println!(
                                                "-> Demande de confirmation reçue : {}",
                                                demand
                                            );
                                            let req_id =
                                                CONFIRMATION_ID.fetch_add(1, Ordering::SeqCst);
                                            let (tx, rx) = tokio::sync::oneshot::channel();

                                            // Ajout du sender dans l'état
                                            if let Some(s) = app.try_state::<AppState>() {
                                                let mut pending =
                                                    s.pending_confirmations.lock().await;
...
async fn connect_mcp_server(
    ...
    println!(
        "Init MCP Server: {} {} {} (auto_approve: {})",
        name, mcp_type, target, auto_approve
    );
    if mcp_type == "stdio" {
        state
            .mcp_manager
            .connect_stdio(name, target, auto_approve)
            .await
    } else {
            ...
            if let Some(servers) = settings.get("mcp_servers").and_then(|s| s.as_array()) {
                for srv in servers {
                    if let (Some(name), Some(t), Some(target)) = (
                        ...
                        tauri::async_runtime::spawn(async move {
                            println!(
                                "Auto-Init MCP Server: {} {} {} (auto_approve: {})",
                                name_c, t_c, target_c, global_auto_approve
                            );
                            if t_c == "stdio" {
                                let _ = mcp_manager
                                    .connect_stdio(name_c, target_c, global_auto_approve)
                                    .await;
                            }
                        });
                    }
